...
from cloudbaseinit.metadata.services import doservice

CONF = cfg.CONF


class DigitalOceanServiceTest(unittest.TestCase):
    def setUp(self):
        CONF.set_override('retry_count_interval', 0)
        self._service = doservice.DigitalOceanService()

    @mock.patch('cloudbaseinit.metadata.services.doservice.DigitalOceanService'
                '.get_host_name')
    def _test_load(self, mock_get_host_name, 
                   side_effect):
    ...
    def test_get_data(self, mock_get_response, mock_Request):
        ...
        fake_path = posixpath.join(CONF.do_metadata_base_url, 'fake')
    ...
    def test_get_host_name(self, mock_get_cache_data):
        ...
        mock_get_cache_data.assert_called_once_with(
            '%s/hostname' % self._service._metadata_version)
    ...
    def test_get_instance_id(self, mock_get_cache_data):
        ...
        mock_get_cache_data.assert_called_once_with(
            '%s/id' % self._service._metadata_version)
        self.assertEqual(mock_get_cache_data.return_value, response)

    @mock.patch('cloudbaseinit.metadata.services.doservice.DigitalOceanService'
                '._get_cache_data')
    def test_get_public_keys(self, mock_get_cache_data):
        mock_get_cache_data.side_effect = ['key', 'fake key']
        response = self._service.get_public_keys()
        expected = [
            mock.call('%s/public-keys' %
